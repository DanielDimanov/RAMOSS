...
import pdb
def DatasetFromSequenceCityScapesFloat(sequenceClass, stepsPerEpoch, nEpochs, batchSize, dims=[256,256,3], out_dims=[256,256,20], data_type=tf.float32, label_type=tf.float32, normalize=True):
...
#             shape = tf.cast(tf.shape(image), tf.float32)
            h = tf.cast(dims[0], tf.int32)
            w = tf.cast(dims[1], tf.int32)
