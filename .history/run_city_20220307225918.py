...
#Specify number of runs (used when statistical significance is needed)
...
#Specify number of epochs (if no proxy is used)
if(args.epochs):
  EPOCHS = int(args.epochs)

#Specify maximum number of convolutional layers
if(args.max_conv_layers):
  MAX_CONV_LAYERS = int(args.max_conv_layers)

#Specify if PSS should be used and the number of splits (if used)
if(args.pss):
  PSS = int(args.pss)
else:
  PSS = False

#Toggle NASBENCH mode on and off (depricated -> moved to separate run_nasbench.py script)
if(args.nasbench):
  NASBENCH = True
else:
  NASBENCH = False
...
for run_num in range(NUMBER_OF_RUNS):
  ...
  # Define the genome handler, which in pymoo terms is the problem 
  ...
  #Feed the data in the genome handler(problem)
  problem.feed_data(
